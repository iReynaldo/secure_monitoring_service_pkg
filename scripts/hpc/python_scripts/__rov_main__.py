...
def scenario_kwargs():
    settings = {
        ...
        "adoption_subcategory_attrs": ("stub_or_mh_ases",), # ("stub_or_mh_ases", "etc_ases", "input_clique_ases"),  # ("stub_or_mh_ases",),  
        "relay_asns": None,
...
def simulation_kwargs():
    return {
        ...
        "parse_cpus": 50,
        "python_hash_seed": 0,
        "caida_kwargs": {"csv_path": Path(f"../../aux_files/rov_adoption_{sys.argv[1]}.csv")}
    }
...
def main():
    ...
    sims = [
        V4Simulation(scenarios=[V4SubprefixHijackScenario(AdoptASCls=Cls,
                     ...
                     output_path=BASE_PATH / f"hpc_subprefix_rov_{sys.argv[1]}_5_attackers_500_trials",
                     **simulation_kwargs()),
    ]
