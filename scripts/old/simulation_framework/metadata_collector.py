...
collect_as_metadata = (
    False  # Defines control switch of enable/disabling metadata collection for each AS
)
as_csv_filename = ""
...
def write_avoid_list_csv_header():
    ...
    avoid_list_csv_filename = "".join(
        [base_path, "/", output_filename, "_avoid_list_metadata.csv"]
    )
    ...
    write_csv_file_header(
        avoid_list_csv_filename, AVOID_LIST_CSV_FIELDNAMES, CSV_FILE_DELIMITER
    )
...
def write_agg_as_csv_header():
    ...
    agg_as_csv_filename = "".join(
        [base_path, "/", output_filename, "_agg_as_metadata.csv"]
    )
    agg_as_csv_lock_filename = "".join([agg_as_csv_filename, ".lock"])
    agg_as_csv_flock = FileLock(agg_as_csv_lock_filename)
    # Write their headers
    write_csv_file_header(
        agg_as_csv_filename, AGG_AS_CSV_FIELDNAMES, CSV_FILE_DELIMITER
    )
