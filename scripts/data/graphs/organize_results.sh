...
for trials in 50 500 2000 8000
do
	...
	do
		...
		do
			...
			for no_relay_zip_file in $( ls | grep tunnel | grep 8000 | grep ${trials}_trials| grep $attack | grep $rov_setting | grep None_relay | grep zip )
			do
			...
			do
				...
				do
					...
					for relay_zip_file in $( ls | grep tunnel | grep 8000 | grep ${trials}_trials | grep $attack | grep ${rov_setting}_rov | grep ${relay_attack}_attackRelay | grep ${relay}_relay | grep zip )
					do
