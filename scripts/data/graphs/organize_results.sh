...
declare -A dirs
dirs=( ["V4SuperprefixPrefixHijack"]="superprefix" ["V4SubprefixHijackScenario"]="subprefix" ["SubprefixAutoImmuneScenario_scenario_direct_type"]="autoimmune-direct" ["SubprefixAutoImmuneScenario_scenario_indirect_type"]="autoimmune-indirect" ["V4PrefixHijackScenario"]="prefix" ["real"]="rov-real" ["none"]="rov-none" ["v4"]="v4-mix" ["True"]="attack-relay" ["False"]="no-attack-relay" ["50"]="prealpha" ["500"]="alpha" ["2000"]="beta" [8000]="final")

for trials in 50 500 2000 8000 16000
do
	# Create a Folder for each of the different settings
	for attack in V4SubprefixHijackScenario SubprefixAutoImmuneScenario_scenario_direct_type SubprefixAutoImmuneScenario_scenario_indirect_type V4SuperprefixPrefixHijack V4PrefixHijackScenario
	do
		for rov_setting in real none v4
		do
			# Move the no-relay setting
			for no_relay_zip_file in $( ls | grep tunnel | grep -E '8000|16000' | grep ${trials}_trials| grep $attack | grep $rov_setting | grep None_relay | grep zip )
			do
			...
			do
				...
				do
					...
					for relay_zip_file in $( ls | grep tunnel | grep -E '8000|16000' | grep ${trials}_trials | grep $attack | grep ${rov_setting}_rov | grep ${relay_attack}_attackRelay | grep ${relay}_relay | grep zip )
					do
