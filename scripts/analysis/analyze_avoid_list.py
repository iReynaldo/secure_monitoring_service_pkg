...
merged_data["avoidList"] = merged_data.avoidList.apply(
    lambda s: set(ast.literal_eval(s) if pd.notnull(s) else set([0]))
)
merged_data["legitPrefixASPath"] = merged_data.legitPrefixASPath.apply(
    lambda s: set(ast.literal_eval(s)) if pd.notnull(s) else set([0])
)

merged_data["intersection"] = merged_data.apply(
    lambda x: x.avoidList.intersection(x.legitPrefixASPath)
    if pd.notnull(x.avoidList) and pd.notnull(x.legitPrefixASPath)
    else np.nan,
    axis=1,
)
