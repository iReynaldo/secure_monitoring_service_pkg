...
#%% Aggregate
data_agg = data.groupby(['percentage', 'propagation_round', 'adoption_setting', 'adopting']).agg({'num_providers': 'mean',
                                                                                                  'empty_rib': 'mean'}).reset_index()
...
for policy in ('ROV V4 Lite', 'ROV++ V1 Lite Simple Overlayed'):
    for adopting in (True, False):
        ...
        print(f"Fraction of ASes with empty Local-RIB: {sum(df.empty_rib) / len(df.empty_rib)}")
        # Fraction of ASes with no provider
        print(f"Fraction of ASes with no provider: {sum(df.num_providers == 0) / len(df.num_providers)}\n")
