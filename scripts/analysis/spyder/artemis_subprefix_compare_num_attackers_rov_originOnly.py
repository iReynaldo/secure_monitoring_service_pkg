...
scenario_type = 'originOnly' # 'none' # 'originOnly'
...
for rov_conf in [20,]:
    for adoption_setting in [dm.all_adoption_setting]:
        for metric in [dm.attacker_success, dm.victim_success, dm.disconnections]:
            ...
            for num_attackers in (1, 20):
                for rov_setting in rov_settings:
                    line_styles_map[index] = dm.lines_style_mapper('artemisOriginOnly', 'cloudflare', rov_setting=rov_setting, num_attackers=num_attackers)
                    index += 1             
