    ...
    for adoption_setting in [dm.adopting_setting, dm.non_adopting_setting]:
        for rov_setting in rov_settings:
            for metric in [dm.attacker_success, dm.victim_success, dm.disconnections]:
                ...
                # Load Results
                rov_results = dm.get_results(
                    standard_policy_paths, subgraph, [dm.policy_name_map["rov"]]
                )
                rovpp_results = dm.get_results(
                    standard_policy_paths, subgraph, [dm.policy_name_map["rovppv1lite"]]
                )
                v4_results = dm.get_results(
                    paths, subgraph, [dm.policy_name_map[bgp_immunity_policy]]
                )
                ...
                for i, policy in enumerate(policy_lines):
                    if policy in dm.standard_policies:
                        ...
                        line_styles_map[i] = dm.lines_style_mapper(
                            bgp_immunity_policy, policy
                        )
                ...
                for i, result in enumerate(results):
                    if result:
                        lines.append(
                            Line(line_styles_map[i], False, result.adopting[subgraph])
                        )
                ...
                mixed_setting = (
                    f"rov_{rov_setting}" if rov_setting != "v4" else "policy_mixed"
                )
                adoption_setting_str = (
                    "non_adopting_"
                    if adoption_setting == dm.non_adopting_setting
                    else ""
                )
                attack_relay_str = "attack" if attack_relay else "with"
                scenario_str = (
                    "prefix" if scenario == "V4PrefixHijackScenario" else "subprefix"
                )
