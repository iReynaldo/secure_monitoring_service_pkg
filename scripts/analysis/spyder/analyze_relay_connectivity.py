...
# Data configuration
# relay = "twenty"
for relay in ('neustar', 'verisign', 'twenty'):
    rov_setting = "none"
    num_trials = 12000
    tunnel = True
    policy = "v4"
    ...
    data_file_path = f"/media/uconn/Seagate Backup Plus Drive/Archives/school_stuff/bgp_immunity/metadata/V4SubprefixHijackScenario_scenario_none_type_others_policies_{rov_setting}_rov_0_hash_False_probe_{tunnel}_tunnel_{relay}_relay_False_attackRelay_1_attacker_{num_trials}_trials_full_percentages_avoid_list_metadata.csv"
    # /media/uconn/Seagate Backup Plus Drive/Archives/school_stuff/bgp_immunity
    ...
    data_file_path_attacked = f"/media/uconn/Seagate Backup Plus Drive/Archives/school_stuff/bgp_immunity/metadata/V4SubprefixHijackScenario_scenario_none_type_others_policies_{rov_setting}_rov_0_hash_False_probe_{tunnel}_tunnel_{relay}_relay_True_attackRelay_1_attacker_{num_trials}_trials_full_percentages_avoid_list_metadata.csv"
    ...
    # Make Plot
    colors = ("blue", "red")
    labels = (f"{policy_label_name[relay]} Relay Not Attacked", f"{policy_label_name[relay]} Relay Attacked")
    marker = {
        'verisign': '.',
        'neustar': 'x',
        'twenty': '*'
    ...
    for i, attack_relay_setting in enumerate((False, True)):
        df = relay_data_agg[(relay_data_agg.attack_relay == attack_relay_setting) & (relay_data_agg.adoption_setting == dm.policy_name_map[policy])]
        _ax.errorbar(
            df.percentage * 100,
            df[feature + "mean"] * 100,
            yerr=df[feature + "calc_90_per_conf"] * 100,
            marker=marker[relay],
            linestyle=line_style[relay],
            color=colors[i],
            label=labels[i],
        )
    ...
    plt.savefig(
        f"./immunity_paper_png_plots/pheme/subprefix/rov_{rov_setting}/relay_usage_{relay}.png",
        bbox_inches="tight",
    )
    # plt.close(_fig)
