...
for adoption_setting in (True, False):
    for outcome_setting in (Outcomes.HIJACKED, Outcomes.VICTIM_SUCCESS, Outcomes.DISCONNECTED):
        ...
        data_agg = data.groupby(['percentage', 'propagation_round', 'adoption_setting']).agg({adopting_all_providers: ['mean', dm.calc_90_per_conf],
                                                                                              adopting_al1ad_providers: ['mean', dm.calc_90_per_conf],
                                                                                              adopting_no_providers: ['mean', dm.calc_90_per_conf]}).reset_index()
        ...
        labels = ('All Providers Adopting', 'At Least 1 Adopting Provider', 'No Adopting Providers')
        for i, feature in enumerate((adopting_all_providers, adopting_al1ad_providers, adopting_no_providers)):
            df = data_agg[data_agg.adoption_setting == dm.policy_name_map[bgp_immunity_policy]]
