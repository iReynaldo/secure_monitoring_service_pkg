...
from v4_graph_generator import Line, generate_plot, compare_relays_linemap, generate_plotly
...
for relays in [('akamai', 'cloudflare', 'verisign', 'incapsula', 'neustar', 'conglomerate'), ('five', 'ten', 'twenty', 'forty')]:
    for metric in [dm.attacker_success, dm.victim_success, dm.disconnections]:
        ...
        for relay in relays:
            paths.append(
                    dm.json_file(scenario, scenario_type, simulated_policies, rov_setting, hash_seed, probe, relay, attack_relay, num_attackers, num_trials, tunnel=tunnel)
                )
