...
# Parameter
bgp_immunity_policy = 'artemis' # 'v4' # 'rovppo'
...
data_file_path_3 = '/media/uconn/Seagate Backup Plus Drive/Archives/school_stuff/bgp_immunity/metadata/ArtemisSubprefixHijackScenario_scenario_originOnly_type_artemis_policies_real_rov_20_conf_True_turnover_0_hash_False_probe_True_tunnel_cloudflare_relay_False_attackRelay_5_attacker_500_trials_full_percentages_agg_as_metadata.csv'
...
data_3 = pd.read_csv(data_file_path_3, delimiter='\t')
...
for adoption_setting in (True, False):
    for outcome_setting in (Outcomes.HIJACKED, Outcomes.VICTIM_SUCCESS, Outcomes.DISCONNECTED):
        ...
        for data, relay in ((data_1, 'cloudflare'), (data_2, 'five'), (data_3, 'cloudflare')):
        ...
        data_agg_1 = data_1.groupby(['percentage', 'propagation_round', 'adoption_setting']).agg({adopting_all_providers: ['mean', dm.calc_90_per_conf],
                                                                                                adopting_al1ad_providers: ['mean', dm.calc_90_per_conf]}).reset_index()
        data_agg_1.columns = list(map(''.join, data_agg_1.columns.values))
        data_agg_2 = data_2.groupby(['percentage', 'propagation_round', 'adoption_setting']).agg({adopting_all_providers: ['mean', dm.calc_90_per_conf],
                                                                                                adopting_al1ad_providers: ['mean', dm.calc_90_per_conf]}).reset_index()
        data_agg_2.columns = list(map(''.join, data_agg_2.columns.values))
        data_agg_3 = data_3.groupby(['percentage', 'propagation_round', 'adoption_setting']).agg({adopting_all_providers: ['mean', dm.calc_90_per_conf],
                                                                                                adopting_al1ad_providers: ['mean', dm.calc_90_per_conf]}).reset_index()
        data_agg_3.columns = list(map(''.join, data_agg_3.columns.values))
        ...
        for i, feature in enumerate((adopting_all_providers, adopting_al1ad_providers)):
            ...
            df_3 = data_agg_3[data_agg_3.adoption_setting == dm.policy_name_map[bgp_immunity_policy]]
            _ax.errorbar(df_1.percentage*100, 
                         df_1[feature+'mean']*100, 
                         yerr=df_1[feature+'calc_90_per_conf']*100,
            ...
            _ax.errorbar(df_2.percentage*100, 
                         ...
                         yerr=df_2[feature+'calc_90_per_conf']*100,
                         marker=markers['five'],
                         linestyle=line_stype[i],
                         color=colors['five'],
                         label=label_prefix['five']+labels[i])
            _ax.errorbar(df_3.percentage*100, 
                         df_3[feature+'mean']*100, 
                         yerr=df_3[feature+'calc_90_per_conf']*100,
                         marker=markers['originOnly'],
                         linestyle=line_stype[i],
                         color=colors['originOnly'],
                         label=label_prefix['originOnly']+labels[i])
