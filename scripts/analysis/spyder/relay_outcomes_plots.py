...
def calculate_outcome(
    grouped_data, new_col_name, outcome, outcome_column="outcome_before_relay"
):
    disconnections_agg_by_trial = grouped_data.aggregate(
        {outcome_column: lambda x: x.eq(outcome).mean()}
    )
    disconnections_agg_by_trial = disconnections_agg_by_trial.reset_index()
    disconnections_agg_by_trial = disconnections_agg_by_trial.rename(
        columns={outcome_column: new_col_name}
    )

    gb_disconnections_agg_by_trial = disconnections_agg_by_trial.groupby(
        by=["adoption_setting", "percentage", "relay"]
    )
    disconnections_agg = (
        gb_disconnections_agg_by_trial.aggregate({new_col_name: "mean"}) * 100
    )
...
# %%
gb_percentage_and_relay = data.groupby(
    by=["adoption_setting", "percentage", "relay", "trial"]
)

# Calculate Attacker Success
attacker_success_agg = calculate_outcome(
    gb_percentage_and_relay, "attacker_success", "Outcomes.ATTACKER_SUCCESS"
)

# Compute Disconnections
disconnections_agg = calculate_outcome(
    gb_percentage_and_relay, "disconnections", "Outcomes.DISCONNECTED"
)

# Calculate Successful Connections
successful_connections_agg = calculate_outcome(
    gb_percentage_and_relay, "successful_connections", "Outcomes.VICTIM_SUCCESS"
)
...
for k in k_settings:
    for outcome, agg in [
        ("attacker_success", attacker_success_agg),
        ("successful_connections", successful_connections_agg),
        ("disconnections", disconnections_agg),
    ]:
        fig, ax = plt.subplots()
        # Set Plot Style
        plt.tight_layout()
        ...
        for relay in relays:
            mask = (agg["adoption_setting"] == f"ROV V4 Lite K{k}") & (
                agg["relay"] == relay
            )
            x = agg.loc[mask, "percentage"]
            y = agg.loc[mask, outcome]
            line_label = (
                f"Pheme {relay.capitalize()} - k={k}"
                if relays == CDNS
                else f"Pheme Peer {dm.peer_map[relay]} - k={k}"
            )
            ax.errorbar(
                x,
                y,
        ...
        # plt.show()
        plt.savefig(
            f"./paper_plots/subprefix/rov_{rov_setting}/subprefix_{relay_name}_relay_k_{k}_outcomes_{outcome}.pdf",
            bbox_inches="tight",
        )
