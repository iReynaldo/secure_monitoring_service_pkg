...
disconnected_adopting_ases_trials = adopting_ases_trials.loc[
    adopting_ases_trials["outcome"] == "Outcomes.DISCONNECTED", :
]
successful_adopting_ases_trials = adopting_ases_trials.loc[
    adopting_ases_trials["outcome"] == "Outcomes.VICTIM_SUCCESS", :
]


sinlge_trial_data = disconnected_adopting_ases_trials.loc[
    (disconnected_adopting_ases_trials["trial"] == 1)
    & (disconnected_adopting_ases_trials["percentage"] == 0.4),
    :,
]
successful_single_trial_data = successful_adopting_ases_trials.loc[
    (successful_adopting_ases_trials["trial"] == 1)
    & (successful_adopting_ases_trials["percentage"] == 0.4),
    :,
]

ten_percent_trials = adopting_ases_trials.loc[
    adopting_ases_trials["percentage"] == 0.1, :
]
...
single_trial_prefix_outcome = sinlge_trial_data.loc[
    sinlge_trial_data["prefix_for_outcome"] == sinlge_trial_data["local_rib_prefix"], :
]
...
print(
    disconnected_ases_with_blackhole / len(single_trial_prefix_outcome["asn"].unique())
)
...
successful_single_trial_data_prefix_outcome = successful_single_trial_data.loc[
    successful_single_trial_data["prefix_for_outcome"]
    == successful_single_trial_data["local_rib_prefix"],
    :,
]
...
avoid_list_item_in_success_path = successful_single_trial_data.apply(
    lambda row: any(asn in row.as_path for asn in row.avoid_list), axis=1
)
