...
for metric in [dm.attacker_success, dm.victim_success, dm.disconnections]:
    ...
    rovpp_results = dm.get_results(
        [paths[1]], subgraph, [dm.policy_name_map["rovppv1lite"]]
    )
    v4_results = dm.get_results(
        paths, subgraph, [dm.policy_name_map[x] for x in bgp_immunity_policies]
    )
    ...
    for i, policy in enumerate(["rov", "rovppv1lite"] + relays):
        if policy in dm.cdns:
            ...
            lines_map[
                i
            ] = f"{policy_legend_prefix} Peer {dm.peer_map[policy]} - adopting"
