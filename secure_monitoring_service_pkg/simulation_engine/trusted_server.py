...
class TrustedServer:
    ...
    def __init__(self, max_num_dishonest_nodes: int = 0):
        ...
        self._raw_data: Dict[str, List[Ann]] = defaultdict(list)
        ...
        self.adopters_with_blackhole: Dict[str, Set[int]] = defaultdict(
            set
        )  # per prefix, ASNs in the sets are adopters that have blackholes
    ...
    def receive_report(self, unprocessed_invalid_ann):
        ...
        self._raw_data[unprocessed_invalid_ann.prefix].append(unprocessed_invalid_ann)
    ...
    def create_recs(self):
        ...
        for prefix in self._raw_data:
            if not (
                self.scenario_name == "SubprefixAutoImmuneScenario" and previous_prefix
            ):
    ...
    def update_recs(self, prefix, recompute=True, previous_prefix=None):
        ...
        if self._max_num_dishonest_nodes == 0:
            ...
            if recompute:
                self._recommendations[prefix] = mvdp.get_avoid_list(
                    self.reports_to_path_list(prefix), self._max_num_dishonest_nodes
                )
            else:
                assert (
                    previous_prefix
                ), "Cannot copy avoid list on None value for previous_prefix"
                ...
                self._recommendations[prefix] = copy.copy(
                    self._recommendations[previous_prefix]
                )
