...
from secure_monitoring_service_pkg.simulation_framework.scenarios import (
...
from secure_monitoring_service_pkg.simulation_framework.scenarios.v4_scenario import (
    CDN_RELAY_SETTING,
)
...
class V4Subgraph(Subgraph):
    ...
    def verify_avoid_list(
        ...
        for prefix in scenario.avoid_lists:
            for asn in scenario.avoid_lists[prefix]:
                ...
                if (
                    asn not in scenario.attacker_asns
                    and asn not in scenario.victim_asns
                ):
                    assert (
                        asn not in scenario.attacker_asns
                    ), f"Attacker ASN {asn} shouldn't be checked in verify_avoid_list"
                    assert (
                        asn not in scenario.victim_asns
                    ), f"Attacker ASN {asn} shouldn't be checked in verify_avoid_list"
                    if asn in outcomes:
                        ...
                        if outcomes[asn] == Outcomes.DISCONNECTED and not (
                            isinstance(as_obj, ROVPPV1SimpleAS)
        ...
        if isinstance(scenario, SubprefixAutoImmuneScenario):
            # Update shared_data
            shared_data[
                "num_ases_should_not_be_on_avoid_list"
            ] = num_ases_should_not_be_on_avoid_list
            shared_data[
                "num_victim_providers_on_avoid_list"
            ] = num_victim_providers_on_avoid_list
    ...
    def aggregate_engine_run_data(
        ...
        *,
        ...
        for attacker_ann in scenario.get_attacker_announcements_for_origin():
            if not shared_data.get(
                "set"
            ):  # this gets set in __add_traceback_to_shared_data:
                # {as_obj: outcome}
                outcomes, traceback_asn_outcomes = self._get_engine_outcomes(
                    engine, scenario, attacker_ann
                )
                if scenario.scenario_config.relay_asns and not isinstance(
                    scenario, ArtemisSubprefixHijackScenario
                ):
                    self._recalculate_outcomes_with_relays(
                        scenario,
                        engine,
                        attacker_ann,
                        outcomes,
        ...
        # Aggregate Outcomes
        self._add_traceback_to_shared_data(
            shared_data, engine, scenario, prefix_outcomes
        )

        prefix_with_minimum_successful_connections = (
            self.get_prefix_with_minimum_successful_connections(scenario, shared_data)
        )
        ...
        shared_data[key] = shared_data.get(
            key + f"_{prefix_with_minimum_successful_connections}", 0
        )
        self.data[propagation_round][scenario.graph_label][percent_adopt].append(
            shared_data.get(key, 0)
        )  # noqa

    def write_avoid_list_metadata(
        self,
        trial,
        percent_adopt,
        propagation_round,
        ...
        with metadata_collector.avoid_list_csv_flock:
            with open(self.avoid_list_csv_filename, "a") as csvfile:
                ...
                if scenario.trusted_server_ref:
                    avoid_list = scenario.trusted_server_ref._recommendations[prefix]
                    avoid_list_len = len(avoid_list)
                # Calculate some CSV features
                num_relay_asns = (
                    0
                    if not scenario.scenario_config.relay_asns
                    else len(scenario.scenario_config.relay_asns)
                )
                # Create new row
                row = {
                    "trial": trial,
                    ...
                    "edge_using_relay": after_relay_usage[
                        self.relay_usage_edge_counter_key
                    ],
                    "etc_using_relay": after_relay_usage[
                        self.relay_usage_etc_counter_key
                    ],
                    "input_clique_using_relay": after_relay_usage[
                        self.relay_usage_clique_counter_key
                    ],
                    "before_relay_num_relays_hijacked": before_relay_usage[
                        Outcomes.ATTACKER_SUCCESS
                    ],
                    "before_relay_num_relays_victim_success": before_relay_usage[
                        Outcomes.VICTIM_SUCCESS
                    ],
                    "before_relay_num_relays_disconnected": before_relay_usage[
                        Outcomes.DISCONNECTED
                    ],
                    "before_relay_num_relays_available": before_relay_usage[
                        self.available_relay_counter_key
                    ],
                    "after_relay_num_relays_hijacked": after_relay_usage[
                        Outcomes.ATTACKER_SUCCESS
                    ],
                    "after_relay_num_relays_victim_success": after_relay_usage[
                        Outcomes.VICTIM_SUCCESS
                    ],
                    "after_relay_num_relays_disconnected": after_relay_usage[
                        Outcomes.DISCONNECTED
                    ],
                    "after_relay_num_relays_available": after_relay_usage[
                        self.available_relay_counter_key
                    ],
    ...
    def write_as_metadata(
        self, trial, percent_adopt, propagation_round, scenario, prefix, outcomes
    ):
        ...
        with metadata_collector.agg_as_csv_flock:
            with open(self.agg_as_csv_filename, "a") as csvfile:
                ...
                for as_obj, outcome in outcomes.items():
                    adoption_setting = (
                        "ad"
                        if as_obj.name == scenario.scenario_config.AdoptASCls.name
                        else "nonad"
                    )
                    (
                        provider_setting,
                        using_adopting_provider_setting,
                    ) = self._provider_setting(as_obj, scenario, prefix)
                    topology_section = self._topology_section(as_obj)
                    ...
                    if (
                        provider_setting == "noad"
                        and outcome == Outcomes.DISCONNECTED
                        and topology_section == "edge"
                    ):
    ...
    def _provider_setting(self, as_obj, scenario, prefix):
        ...
        using_adopting_provider_setting = (
            "using" if using_adopting_provider else "notusing"
        )
    ...
    def _calc_adopting_provider_features(self, as_obj, scenario, prefix):
        ...
        for provider_as_obj in as_obj.providers:
            if isinstance(provider_as_obj, scenario.scenario_config.AdoptASCls):
                ...
                if (
                    most_specific_ann_as_path
                    and provider_as_obj.asn in most_specific_ann_as_path
                ):
    ...
    def _as_has_blackhole_for_attack_on_origin_prefix(self, as_obj, attacker_prefix):
        ...
        if (
            attacker_ann_from_relay_local_rib
            and attacker_ann_from_relay_local_rib.blackhole
        ):
            return True
        return False

    def _relay_is_available(
        self, engine, scenario, outcomes, attacker_prefix, relay_asn
    ):
        """
        ...
        if scenario.scenario_config.probe_data_plane:
            ...
            if origin_ann and not self._as_has_blackhole_for_attack_on_origin_prefix(
                as_obj, attacker_prefix
            ):
                if scenario.trusted_server_ref:
                    # Check that the AS path for the origin prefix does not have any ASes that have blackholes
                    ases_with_blackholes = (
                        scenario.trusted_server_ref.adopters_with_blackhole.get(
    ...
    def _recalculate_outcomes_with_relays(
        self,
        scenario,
        engine,
        attacker_ann,
        ...
        for relay_asn in scenario.scenario_config.relay_asns:
            if self._relay_is_available(
                engine, scenario, outcomes, attacker_ann.prefix, relay_asn
            ):
                available_relays.add(relay_asn)
        # Update Metadata tracking variable
        before_relay_usage.update(
            (self.available_relay_counter_key,) * len(available_relays)
        )
        ...
        if len(available_relays) > 0:
            ...
            for as_obj_iterator in [not_connected_relay_as_obj, outcomes]:
                for as_obj in as_obj_iterator:
                    # Update Metadata tracking variable
                    if (
                        as_obj.asn in scenario.scenario_config.relay_asns
                        and as_obj.asn not in seen_relay_ases
                    ):
                        before_relay_usage.update((outcomes[as_obj],))
                    if (
                        self.has_access_to_relay_service(as_obj)
                        and outcomes[as_obj] != Outcomes.VICTIM_SUCCESS
                        and as_obj.asn not in available_relays
                    ):
                        if (
                            as_obj.asn in scenario.scenario_config.relay_asns
                            and scenario.scenario_config.relay_setting
                            == CDN_RELAY_SETTING
                        ):
                            selected_relay_asn = as_obj.use_relay(
                                available_relays, scenario.relay_prefixes, True
                            )
                        else:
                            selected_relay_asn = as_obj.use_relay(
                                available_relays,
                        ...
                        if selected_relay_asn:
                            ...
                            outcomes[as_obj] = outcomes[
                                engine.as_dict[selected_relay_asn]
                            ]
                            # Update traceback_asn_outcome to victim asn
                            traceback_asn_outcomes[as_obj.asn] = traceback_asn_outcomes[
                                selected_relay_asn
                            ]
            ...
            for as_obj_iterator in [not_connected_relay_as_obj, outcomes]:
                for as_obj in as_obj_iterator:
                    if (
                        self.has_access_to_relay_service(as_obj)
                        and outcomes[as_obj] == Outcomes.ATTACKER_SUCCESS
                        and as_obj.asn not in available_relays
                        and scenario.scenario_config.probe_data_plane
                    ):
    ...
    def get_prefix_with_minimum_successful_connections(self, scenario, shared_data):
        ...
        for attacker_ann in scenario.get_attacker_announcements_for_origin():
            if attacker_ann.prefix not in scenario.relay_prefixes.values():
                ...
                subgraph_key = (
                    f"all_{Outcomes.VICTIM_SUCCESS.name}_perc_{attacker_ann.prefix}"
                )
    ...
    def _get_as_outcome(
        ...
        if (
            force_recompute
            and as_obj in outcomes
            and outcomes[as_obj] != Outcomes.DISCONNECTED
        ):
        ...
        if not force_recompute and as_obj in outcomes:
            ...
            outcome, traceback_asn = scenario.determine_as_outcome(
            ...
            if outcome == Outcomes.UNDETERMINED:
                ...
                outcome, traceback_asn = self._get_as_outcome(
                    next_as,
                    outcomes,
                    traceback_asn_outcomes,
                    engine,
    ...
    def _get_most_specific_ann(
        ...
        # NOTE to Reynaldo about this speedup
        # ipaddress func are VERY slow. These two lines I commented out
        # were taking up a huge portion of the runtime of your program
        # That is the purpose of the ordered_prefixes dict, to avoid
        # subnet_of operations
        # attacker_ann_prefix = ipaddress.ip_network(attacker_ann_prefix)
        assert attacker_ann_prefix in ordered_prefixes, "Must use commented out method"
        for prefix, subprefixes in ordered_prefixes.items():
            if attacker_ann_prefix in subprefixes or attacker_ann_prefix == prefix:
                ...
                if most_specific_ann:
                    ...
                    if (
                        len(most_specific_ann.as_path) > 1
                        and most_specific_ann.as_path[1] in provider_asns
                    ):
                        # Mypy doesn't recognize that this is always an annoucnement
                        return most_specific_ann  # type: ignore
                    else:
    ...
    def _get_engine_outcomes(
        ...
        recompute_disconnections: bool = False,
        ...
        traceback_asn_outcomes: Dict[int, int] = (
            traceback_asn_outcomes if traceback_asn_outcomes else dict()
        )
        for as_obj in engine.as_dict.values():
            if recompute_disconnections:
                if outcomes[as_obj] == Outcomes.DISCONNECTED and not isinstance(
                    as_obj, scenario.scenario_config.AdoptASCls
                ):
                    # Gets AS outcome and stores it in the outcomes dict
                    self._get_as_outcome(
                        as_obj,
                        outcomes,
                        traceback_asn_outcomes,
                        engine,
    ...
    def _add_traceback_to_shared_data(
        ...
        for prefix, outcomes in prefix_outcomes.items():
            for as_obj, outcome in outcomes.items():
                ...
                as_type_pol_prefix_outcome_perc_k = (
                    self._get_as_type_pol_prefix_outcome_perc_k(
    ...
    def _get_as_type_pol_prefix_outcome_k(
        self, as_type: Any, ASCls: Type[AS], prefix: str, outcome: Outcomes
    ) -> str:
        """returns as type+policy+outcome key"""
        x = self._get_as_type_pol_outcome_k(as_type, ASCls, outcome)
        return f"{x}_{prefix}"

    def _get_as_type_pol_prefix_outcome_perc_k(
        self, as_type: Any, ASCls: Type[AS], prefix: str, outcome: Outcomes
    ) -> str:
