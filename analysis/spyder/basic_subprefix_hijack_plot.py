...
for cdn in ['akamai', 'cloudflare', 'verisign', 'incapsula', 'neustar']:
    ...
    for num_attackers in [1, 2, 5]:
        paths.append(f'../../data/graphs/jsons/artemis_{cdn}_cdn_{num_attackers}_attacker_50_trials.json')
    ...
    for path in paths:
        ...
        with open(path, "r") as json_file:
            ...
            result = PolicyResult("v4_attacker_success_all", "0", "BGP Simple", "ARTEMIS", json_data)
            results.append(result)
    
    lines = []
    capitalized_cdn = cdn.capitalize()
    lines_map = {0: f"ARTEMIS {capitalized_cdn} - 1 Attackers", 
                 1: f"ARTEMIS {capitalized_cdn} - 2 Attackers", 
                 2: f"ARTEMIS {capitalized_cdn} - 5 Attackers"}
    
    for i, result in enumerate(results):
        lines.append(Line(lines_map[i], False, result.non_adopting["v4_attacker_success_all"]))
