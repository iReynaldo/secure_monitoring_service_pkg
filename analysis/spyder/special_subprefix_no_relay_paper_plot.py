...
# It's intended purpose is to plot just 3 curves, ROV, V1Lite, Pheme K=0 for subprefix hijack, no relay, no mixed ROV
####################################
# Main
####################################
...
for metric in [dm.attacker_success, dm.victim_success, dm.disconnections]:
    ...
    # Load Results
    policy_key_values = [policy_name_map[x] for x in policies]
    ...
    generate_plot(lines,
                  ...
                  fname=f"./paper_plots/subprefix/rov_{rov_setting}/basic_subprefix_no_relay_{dm.metric_filename_prefix[metric]}.pdf")
