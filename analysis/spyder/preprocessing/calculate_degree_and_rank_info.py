...
ranks_dict = dict()
ases_sorted_by_customer_conse_size = sorted(customer_cone_size_dict, key=customer_cone_size_dict.get, reverse=True)
for asn in customer_cone_size_dict:
    ranks_dict[asn] = ases_sorted_by_customer_conse_size.index(asn) + 1  # largest customer cone size is 1 (not 0)
...
with open('../../../data/graphs/metadata/asn_degree_and_rank.csv', 'w') as csvfile:
    ...
    for asn in degree_dict:
        rows.append([asn, degree_dict[asn], customer_cone_size_dict[asn], ranks_dict[asn]])
    # Write rows
    print("Writing CSV")
